...
import { Gamepad2, Plus, Trash2, Pencil, Check, X, Clock, Target, Flame, MapPin } from "lucide-react";
...
// Reusable CRUD list for simple config tables
const ConfigList = ({
  title,
  icon,
  items,
      ...
      <CardContent className="space-y-3">
          ...
          <div key={item.id} className="flex items-center gap-2 rounded-lg border border-border bg-secondary/30 px-3 py-2.5">
            {editingId === item.id ? (
                ...
                <div className="flex-1 flex gap-2">
                  ...
                  {hasValue && (
                    <Input
                      ...
                      className="h-8 bg-background w-36 font-mono text-xs"
                      placeholder="value"
                      onKeyDown={(e) => e.key === "Enter" && handleSaveEdit(item.id)}
                    />
                  )}
              ...
              <>
                ...
                <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => { setEditingId(item.id); setEditLabel(item.label); setEditValue(item.value || ""); }}>
        ...
        <div className="flex gap-2 pt-2">
          <Input
            ...
            value={newLabel}
            onChange={(e) => { setNewLabel(e.target.value); if (hasValue) setNewValue(autoValue(e.target.value)); }}
          ...
          {hasValue && (
            <Input
              ...
              className="h-10 bg-secondary border-border w-36 font-mono text-xs"
              onKeyDown={(e) => e.key === "Enter" && handleAdd()}
            />
          )}
          <Button onClick={handleAdd} disabled={!newLabel.trim()} className="gap-1.5 shrink-0">
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key="myplayer">
      ...
      <p className="text-muted-foreground mb-8">
        Configure all the options customers see when building their player profile.
      </p>
          ...
          <CardContent>
            {locations.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {locations.map(loc => (
                  <span key={loc.id} className="px-3 py-1.5 rounded-lg border border-border bg-secondary/30 text-sm font-medium text-foreground">
                    {loc.name}
                  </span>
                ))}
              </div>
