...
import { BookingRow, ClubRow, UserWithEmail, AuditLogRow, OPEN_HOUR, CLOSE_HOUR, ACTIVITY_OPTIONS, getBookingRevenue } from "./types";
...
const BookingsCalendarTab = ({ bookings, clubs, isMasterAdmin, onDeleteBooking, onUpdateBooking, onAddBooking, allUsers, initialDate, onInitialDateHandled, priceMap, clubActivityMap }: Props) => {
  ...
  return (
    <div className="space-y-6">
      {isMasterAdmin && <PageCmsSection pageSlug="book" pageName="Book a Session" showPictures showFields fieldsLabel="Booking Detail Fields" showMaxClubsGrid />}
      {/* Controls row */}
        ...
        <DialogContent className="bg-card border-border max-w-md">
          ...
          {selectedBooking && (
            <div className="space-y-4 pt-2">
              ...
              {!bookingPrice && selectedBooking.price != null && (
                <div className="flex items-center justify-between pt-2 border-t border-border">
                  <div className="flex items-center gap-2">
                    <DollarSign className="h-4 w-4 text-muted-foreground" />
                    <span className="text-xs text-muted-foreground">Price (recorded)</span>
                  </div>
                  <span className="font-semibold text-foreground">
                    {selectedBooking.discount_type === "free"
                      ? <><span className="line-through text-muted-foreground mr-2">${selectedBooking.price}</span><span className="text-emerald-400">FREE</span></>
                      : selectedBooking.discount_type === "50%"
                      ? <><span className="line-through text-muted-foreground mr-2">${selectedBooking.price}</span>${(Number(selectedBooking.price) / 2).toFixed(0)}</>
